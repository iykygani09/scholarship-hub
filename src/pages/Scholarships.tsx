...
import { Plus, Search, Pencil, Trash2, X, Upload, GraduationCap, Loader2 } from "lucide-react";
...
export default function Scholarships() {
  ...
  const openEdit = (p: InternalProgram) => {
    setEditItem(p);
    setForm({ title: p.title, description: p.description, category: p.category, eligibility: p.eligibility, benefit: p.benefit, amount: p.amount, totalSeats: p.totalSeats, deadline: "" });
    setImageFile("");
    setShowModal(true);
  };
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left p-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden lg:table-cell">Seats</th>
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="p-4 text-muted-foreground hidden lg:table-cell">{p.filledSeats}/{p.totalSeats}</td>
                    <td className="p-4">
                      <span className={`text-xs px-3 py-1 rounded-full ${p.status === "Active" ? "status-approved" : p.status === "Upcoming" ? "status-pending" : "status-rejected"}`}>
                        {p.status}
                      </span>
                    </td>
                    <td className="p-4 text-right">
                      <button onClick={() => openEdit(p)} className="text-muted-foreground hover:text-primary p-2 transition-colors" title="Edit">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => setDeleteConfirm(p.id)} className="text-muted-foreground hover:text-destructive p-2 transition-colors" title="Delete">
          ...
          <div className="glass-card max-w-lg w-full p-6 animate-slide-up max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Describe the scholarship..." className="input-dark w-full h-20 resize-none" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="date" value={form.deadline} onChange={(e) => setForm({ ...form, deadline: e.target.value })} className="input-dark w-full" />
                </div>
              </div>
              <div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="px-5 py-2.5 rounded-lg bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-colors">
                Cancel
              </button>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
          <div className="glass-card max-w-sm w-full p-6 animate-slide-up text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-muted-foreground mb-6">This action cannot be undone. The scholarship and all associated data will be permanently removed.</p>
            <div className="flex gap-3 justify-center">
              <button onClick={() => setDeleteConfirm(null)} className="px-5 py-2.5 rounded-lg bg-secondary text-secondary-foreground hover:bg-secondary/80">
                Cancel
              </button>
